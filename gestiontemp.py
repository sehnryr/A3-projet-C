...
class myapp:
    ...
    def init_variables(self):
        ...
        try:
            ...
            print(
                "Can not open ",
                self.datafilename,
                " file for initialization! Program stopped",
            )
            sys.exit()
    ...
    def init_lpanel(self):
        ...
        tk.Checkbutton(
            ...
            onvalue=1,
            offvalue=0,
            command=self.cb_selection,
        ).grid(column=0, row=crow)
        crow += 1
        # ---
        tk.Label(self.lpanel, text="---").grid(column=0, row=crow)
        crow += 1
        ...
        self.btn_StartStop = Button(
            self.lpanel,
            textvariable=self.btn_StartStop_txt,
            command=self.startstopit,
            bg="black",
            fg="white",
        ).grid(column=1, row=crow)
        crow += 1
        # ---
        tk.Label(self.lpanel, text="---").grid(column=0, row=crow)
        crow += 1
        ...
        self.slidebar = tk.Scale(
            self.lpanel,
            variable=self.str_Thermostat,
            from_=30,
            to=0,
            resolution=0.5,
            length=200,
    ...
    def cb_selection(self):
        if self.cbvar.get() == 1:
            ...
            fp.close()
            print(
                "Freeze on (set lock file ",
                self.lockfilenamedata,
                " and ",
                self.lockfilenameconsigne,
                ")",
            )
        else:
    ...
    def cbk_refresh(self):  # Called each time
        ...
        try:
            ...
            print(
                "Lock file ",
                self.lockfilenamedata,
                " exists. Use previous values for plotting Text and Tint",
            )
        else:
            ...
            try:
                ...
                print(
                    "Can not open ",
                    self.datafilename,
                    " file. Use previous values for plotting Text and Tint.",
                )
                Tint = self.oTint
                Text = self.oText
            else:
    ...
    def cbk_thermostat(self, value):  # Called when slider has been modified
        print(
            "Write ",
            self.consignefilename,
            " with ",
            self.str_Thermostat.get(),
            "°C at time ",
            strftime("%H:%M:%S", gmtime(self.ctime * REAL_DELAY_VALUE)),
        )
        # Check lock file and add previous value for the plot if it exists
        try:
            ...
            try:
                ...
                print(
                    "Can not open ", self.consignefilename, " file. It is not written."
                )
            else:
